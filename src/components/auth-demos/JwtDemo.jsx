  ...
  return (
    <div className="bg-surface-alt border border-line rounded p-6">
      <p className="text-muted mb-4">
        ...
        <code>header.payload.signature</code>. The header and payload are
        just base64 - anyone can decode and read them, no secret required.
        What makes a JWT trustworthy is the signature: it's computed from the
        header and payload using a secret only the server knows, so changing
        the payload without the secret breaks the signature.
      </p>
      ...
      <div className="grid md:grid-cols-3 gap-3 mb-4 text-xs font-mono">
        ...
        <div className="bg-surface border border-line rounded p-3">
          ...
          <p className="text-subtle">
            HMAC(header + payload, SECRET) - only the server can produce a
            valid one
          </p>
        </div>
      </div>
