...
function EventLoopDemo() {
  ...
  const run = () => {
    ...
    // synchronous lines first, then queued microtasks (the promise), and
    // only then macrotasks (the timer), even with a 0ms delay.
    setLog((l) => [...l, "1. synchronous log - runs first"]);
    ...
    Promise.resolve().then(() => {
      setLog((l) => [
        ...l,
        "3. .then callback (microtask) - runs before any timer",
      ]);
    });

    setLog((l) => [...l, "2. synchronous log - still runs before async"]);
  };

  return (
    <div className="bg-surface-alt border border-line rounded p-6">
      <p className="text-muted mb-4">
        Click run and watch the order entries actually appear in - not the
        order they&apos;re written in. Every synchronous line finishes first,
        then every queued microtask (promises), and only then macrotasks
        (timers), even a <code>setTimeout(fn, 0)</code>.
      </p>
