...
function RouterDemo() {
  ...
  const resolve = (mount) => {
    ...
    const full =
      mount.prefix + (routePath === "/" ? "" : routePath);
  ...
  return (
    <div className="bg-surface-alt border border-line rounded p-6">
      <p className="text-muted mb-4">
        <code>express.Router()</code> creates a self-contained set of routes
        you can build and test in its own file, then mount at a path prefix
        with <code>app.use(prefix, router)</code>. Every route inside the
        router is relative to that prefix - this is how large apps stay
        organized instead of becoming one giant file of routes.
      </p>
