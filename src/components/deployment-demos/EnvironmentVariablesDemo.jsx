...
const ENV_VARS = [
  ...
  { key: "DATABASE_URL", value: "postgres://user:pass@db.host/app", public: false },
...
function EnvironmentVariablesDemo() {
  ...
  const buildApp = () => {
    ...
    setLog([
      "npm run build",
      ...publicVars.map(
        (v) => `✓ ${v.key} inlined into the shipped JS bundle - anyone can read it in dev tools`,
      ),
      ...secretVars.map(
        (v) => `✓ ${v.key} stays on the server only - never reaches the browser`,
      ),
    ]);
  };
  ...
  return (
    <div className="bg-surface-alt border border-line rounded p-6">
      <p className="text-muted mb-4">
        Environment variables keep config and secrets out of your source
        code. But not all of them are equally secret - it depends on{" "}
        <em>where</em> they're read, and most frameworks mark the
        browser-safe ones with a prefix like <code>VITE_</code> or{" "}
