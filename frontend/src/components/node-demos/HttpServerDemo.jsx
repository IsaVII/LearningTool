...
function HttpServerDemo() {
  ...
  const post = () => {
    ...
    setLog((l) => [
      ...l,
      `POST /quotes { "quote": "${quote}" } -> 201 Saved`,
    ]);
  };
  ...
  return (
    <div className="bg-surface-alt border border-line rounded p-6">
      <p className="text-muted mb-4">
        Each button simulates a client sending a request to the server from
        the example above. The handler branches on <code>req.method</code>{" "}
        and <code>req.url</code> to decide how to respond.
      </p>
